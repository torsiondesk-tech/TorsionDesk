...
- [ ] **AUTH-06**: Role-based access — Admin sees all modules; Dispatcher sees dispatch, jobs, customers; Technician sees assigned jobs, related customer info, estimates linked to those jobs/customers, and invoices/payments for jobs they completed
...
- [ ] **EST-05**: Estimate can be converted to a job with one action from the office or from the PWA — all fields, line items, and contact info carry over to the new job record
...
- [ ] **EST-09**: User or technician can email or text an estimate PDF to the customer directly from the estimate form / PWA estimate view
...
- [ ] **TECH-09**: Tech can view and create estimates in the PWA; created estimates carry customer, contact, service location, line items, and follow-up tasks
- [ ] **TECH-10**: Tech can convert a won estimate to a job in one action from the PWA; all estimate fields, line items, and contact info carry over to the new job record
- [ ] **TECH-11**: Tech can create an invoice from a completed job in the PWA in one action; invoice carries over all line items, customer, and contact details (same outcome as "Close & Invoice" from the dispatch board)
- [ ] **TECH-12**: Tech can email or text estimate and invoice PDFs to the customer directly from the PWA; sending respects the per-trigger automation settings defined in Phase 8
- [ ] **TECH-13**: Tech can collect on-site card payments via Square Mobile Payments SDK from the PWA; payment posts to the same canonical ledger as office-recorded payments
- [ ] **TECH-14**: Offline mutations in the PWA include estimate creation, estimate-to-job conversion, invoice creation, and payment records; all queued items are visually flagged and auto-synced when connectivity is restored
...
- [x] **INV-02**: Invoicing dashboard has AR aging sidebar: Grand Total Unpaid, Grand Total Past Due, aging buckets (1–30 / 31–60 / 61–90 / 91+ days) with color-coded totals
- [x] **INV-03**: Invoice list has three views: Unpaid | Paid | All Invoices — each with search, sort, and pagination
...
- [ ] **INV-07**: Square integration — tech can process on-site card payment via Square Mobile Payments SDK from the PWA on their phone; payment syncs to the canonical payment ledger
...
- [ ] **INV-09**: Invoice can be emailed or texted to customer with PDF attachment directly from the invoice view or the PWA
- [ ] **INV-10**: "Invoice" button appears on the job detail action bar and in the PWA when job status is Completed; clicking it creates an invoice carrying all line items, customer, and contact details, and transitions the job to Invoiced — same outcome as "Close & Invoice" from the dispatch popup
...
- [x] **CAT-01**: Products have: category, name, model, SKU, UPC, part number, unit price, unit cost, type, active flag, inventory item flag, sales description (customer-facing, appears on invoices), purchase description (internal, for ordering), primary/secondary/tertiary vendor each with a purchase price
- [x] **CAT-02**: Services have: category, name, unit price, unit cost, description, active flag
- [x] **CAT-03**: Catalog management page — filterable by category, name, price range, inventory item flag; sortable by name/price/cost; bulk CSV export
- [x] **CAT-04**: Inline product/service creation from job/estimate line item add flow — "Create New Product" or "Create New Service" opens a modal, saves to catalog, and immediately adds as a line item
- [x] **CAT-05**: Product categories are admin-configurable (Garage Doors, Openers, Hardware, Extension Springs, Torsion Springs, Operator Parts, Window Inserts, Misc., etc.)
...
- [ ] **COMM-06**: Invoice email or text is sent to customer when an invoice is created or sent from the office/PWA (if trigger is enabled)
...
- [ ] **COMM-09**: PDF attachments included in all outbound job/estimate/invoice emails and texts; work order PDF optionally included with invoice emails
...
- [x] **SET-01**: Job categories — hierarchical structure (Commercial / Residential top level; sub-types: New Door, New Door + Operator, New Operator, Service/Repair, Springs, etc.); admin can add, edit, delete, reorder
- [x] **SET-02**: Tags — admin can create, edit, and delete custom tags; tags apply to both jobs and estimates; appear in sidebar navigation with counts
- [x] **SET-03**: Job and estimate templates — admin can create, edit, and delete templates with pre-filled line items and task lists; applied at job/estimate creation
- [x] **SET-04**: User management — admin can add users by email, assign roles (Admin, Dispatcher, Technician), deactivate users
- [x] **SET-05**: Email automation settings — per-trigger enable/disable toggles for all outbound email types; editable email body templates
- [x] **SET-06**: Lookup lists — admin can add, edit, and remove referral sources, job sources, and other configurable dropdown values
- [x] **SET-07**: Tax items — admin can create and configure tax rates (name, percentage); tax items are assigned per customer (taxable flag) and overridable per line item
...
| CAT-01 | Phase 2 | Delivered |
| CAT-02 | Phase 2 | Delivered |
| CAT-03 | Phase 2 | Delivered |
| CAT-04 | Phase 2 | Delivered |
| CAT-05 | Phase 2 | Delivered |
| CAT-06 | TBD | Deferred |
| SET-01 | Phase 2 | Delivered |
...
| TECH-10 | Phase 5 | Pending |
| TECH-11 | Phase 5 | Pending |
| TECH-12 | Phase 5 | Pending |
| TECH-13 | Phase 5 | Pending |
| TECH-14 | Phase 5 | Pending |
